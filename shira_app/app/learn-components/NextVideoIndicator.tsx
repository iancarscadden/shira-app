...
interface NextVideoIndicatorProps {
    ...
    isVideoReady?: boolean; // New prop to track video ready state
}
...
const NextVideoIndicator: React.FC<NextVideoIndicatorProps> = ({
    gestureY,
    isActive,
    height,
    direction,
    ...
    useEffect(() => {
        if (isActive && direction === 'up') {
            ...
            cardTranslateY.setValue(20 - (progress * 20));
            
            // Reset animation state
            animationState.isAnimatingUp = false;
            animationState.hasTriggeredNextVideo = false;
        ...
        } else if (!isActive && direction === 'up' && gestureY > 100) {
            ...
            // Set animation state
            animationState.isAnimatingUp = true;
            animationState.hasTriggeredNextVideo = true;
        ...
        } else if (!isActive && !animationState.hasTriggeredNextVideo) {
    ...
    useEffect(() => {
        if (isVideoReady && animationState.hasTriggeredNextVideo) {
            // When video is ready and we've triggered the next video, animate out

            // Animate gradient off screen
            Animated.parallel([
                Animated.timing(gradientPosition, {
    ...
    // Don't render if swiping down
    if (direction === 'down') return null;
    
    // Calculate translation value for gradient container
    const translateY = gradientPosition.interpolate({
        inputRange: [0, 1, 2],
        outputRange: [height, 0, -height], // Move from bottom to top to off-screen
        extrapolate: 'clamp',
    });
                    ...
                    <Animated.View
                        ...
                        style={[
                            styles.particle,
                            {
                                ...
                                top: height / 2 - 50 + particle.initialY,
                ...
                >
                    ...
                    >
                        <BlurView intensity={30} tint="dark" style={styles.cardBlur}>
                            ...
                            <Text style={styles.text}>Next Video</Text>
                            
                            {/* Dynamic message that changes with progress */}
                            {/* Removing the subtext as requested */}
                        </BlurView>
                    </Animated.View>
                </Animated.View>
            </Animated.View>
...
export default NextVideoIndicator; 